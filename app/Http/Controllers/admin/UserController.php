...
class UserController extends Controller
{
    ...
    public function user_store(Request $request)
    {
        ...
        if ($request->hasFile('avatar')) {
            ...
            $user->avatar = saveImage($avatar, '/uploads/avatars','');
        }

        $user->save();

        return redirect()->route('admin.user')->with('success', 'Người dùng đã được thêm');
    }
    ...
    public function user_update(Request $request)
    {
        ...
        if ($user) {
            $user->role = $request->role;
            $user->save();
            return redirect()->route('admin.user')->with('success', 'Người dùng đã được cập nhật');
        } else {
    ...
    public function user_delete($userID)
    {
        ...
        if ($user) {
            if ($user->userID === Auth::user()->userID) {
                return redirect()->route('admin.user')->with('error', 'Không thể xóa chính mình');
            }
            ...
            $user->delete();
            return redirect()->route('admin.user')->with('success', 'Người dùng đã được xoá');
        } else {
            return redirect()->route('admin.user')->with('error', 'Không tìm thấy người dùng');
        }
    }
